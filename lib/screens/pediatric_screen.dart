...
class _PediatricScreenState extends State<PediatricScreen> {
  ...
  // 固定長: 身長/11 + 5.5 cm, 0.5 cm 単位
  double get _fixLen => ((_height / 11.0 + 5.5) * 2).round() / 2.0;
  ...
  // アトロピン (≤10 kg のみ, 10 倍希釈 0.05 mg/mL)
  ...
  Widget _tubeRow({
    ...
    const sm = TextStyle(fontSize: 12, color: Colors.black54);
    return Row(
      crossAxisAlignment: CrossAxisAlignment.baseline,
      textBaseline: TextBaseline.alphabetic,
      children: [
        SizedBox(width: 58, child: Text(rowLabel, style: sm)),
        Text(leftLabel, style: sm),
        const SizedBox(width: 4),
        Text(leftVal.toStringAsFixed(1), style: big),
        Text(' $unit', style: sm),
        Text('  /  $rightLabel ', style: sm),
        Text(rightVal.toStringAsFixed(1), style: big),
        Text(' $unit', style: sm),
      ],
    );
  }
