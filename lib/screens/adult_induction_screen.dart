...
class _AdultInductionScreenState extends State<AdultInductionScreen> {
  ...
  double get _etTarget => _volatile == _Volatile.sevoflurane
      ? 1.7 - 0.01 * _age
      : 6.0 - 0.04 * _age;
  String get _etLabel => _volatile == _Volatile.sevoflurane ? 'etSev' : 'etDes';
  ...
  _DoseInfo get _sedInfo {
    ...
    switch (_sed) {
      ...
      case _SedDrug.remimazolam:
        return _DoseInfo(
          [
            _DoseLine('導入（減速）', wt * 6.0, 'mg/h', '6 mg/kg/h'),
            _DoseLine('導入（標準）', wt * 12.0, 'mg/h', '12 mg/kg/h'),
            _DoseLine('維持開始', wt * 1.0, 'mg/h', '1 mg/kg/h'),
          ],
          '導入は意識消失まで持続注入する. ボーラス導入ではない. ASA-PS 3以上などでは6 mg/kg/hを考慮. 維持は1 mg/kg/hで開始し調節する.',
        );
  ...
  String get _sedMlStr {
    ...
    switch (_sed) {
      ...
      case _SedDrug.remimazolam:
        return '${_fmtMl(wt * 6.0)} 〜 ${_fmtMl(wt * 12.0)} ml/h (導入)';
      case _SedDrug.midazolam:
  ...
  String? get _sedMlLine2 {
    ...
    return '${_fmtMl(_wt * 1.0)} ml/h (維持開始)';
  }

  String get _sedConcNote => switch (_sed) {
    _SedDrug.propofol => '10 mg/ml',
    _SedDrug.remimazolam => '1 mg/ml調製 · 導入12 mg/kg/h（減速時6）· 維持1 mg/kg/h',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '年齢・身長・体重・性別は「機能」TOPの患者情報から自動連携',
        ...
        child: Column(
          ...
          children: [
            ...
            _mlRow(
              ...
              line1:
                  '${_volatile.label}  導入 ${_volatile.initPct.toStringAsFixed(0)}%',
  ...
  // ── helpers ─────────────────────────────────────────────────────────────
  Widget _secTitle(String text, ColorScheme scheme) => Text(
    text,
    style: TextStyle(
      color: scheme.primary,
      fontWeight: FontWeight.bold,
